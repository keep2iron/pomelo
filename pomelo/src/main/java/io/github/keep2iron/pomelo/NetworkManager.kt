...
class NetworkManager private constructor() {
    ...
    class Builder constructor(
        ...
        fun initOkHttp(url: String = defaultUrl, block: OkHttpClient.Builder.() -> Unit) {
            check(urlList.indexOf(url) != -1) { "do you forget call addBaseUrl at first." }

            okhttpBuilderMap[url]!!.apply(block)
        }

        fun initRetrofit(url: String = defaultUrl, block: Retrofit.Builder.() -> Unit) {
            check(urlList.indexOf(url) != -1) { "do you forget call addBaseUrl at first." }
        ...
        fun build(): NetworkManager {
            ...
            urlList.forEach { url ->
                ...
                val retrofit = retrofitBuilderMap[url]!!.client(okHttpClient).baseUrl(url).build()
